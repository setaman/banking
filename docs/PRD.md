...
| F15 | Provider Configuration (BYOK) | Settings card to choose a provider (OpenAI, Anthropic, Google Gemini, or Ollama for a fully local/offline setup) and model, with a "Test Connection" check. Stored only in the local, gitignored `banking.config.json` — never committed, never sent anywhere but the chosen provider. |
...
│   │   ├── backup.ts               # Pre-sync backup & OperationResult type
│   │   ├── storage.ts              # DB file path constants
...
2. **Create backup of current DB state** (flush in-memory data, copy file to `data/db-backup.json`)
3. For each account, fetch transactions since last sync date
4. Map responses through DKB adapter (Zod validation)
5. Deduplicate by SHA256 hash of key fields
6. Persist new transactions + current balances to LowDB

### 4.4 Backup & Recovery

- **Automatic pre-sync backup**: Before each sync, the current DB file is copied to `data/db-backup.json`
- **Schema-validated restore**: `restoreFromBackup()` server action validates backup against `DatabaseSchema` before accepting
- **Non-blocking**: Backup failures log a warning but never prevent sync from proceeding
- **Single-slot**: Only the most recent pre-sync state is preserved (last sync's backup)
...
1.  **Dashboard (Home)**
    - _Primary Goal:_ Instant financial health check.
    - _Included KPIs:_
      - **K1 Monthly Cash Flow** (Trend Bar)
      - **K2 Savings Rate** (Radial)
      - **K10 MoM Trend** (Indicator)
      - **K12 Daily Average** (Stat card)
    - _Components:_ Total Balance Chart, Recent Activity List.
2.  **Accounts**
    - _Primary Goal:_ Manage sources of funds.
    - _Components:_ List of bank connections, individual account balances/details.
3.  **Transactions**
    - _Primary Goal:_ Deep dive and categorization.
    - _Included KPIs:_
      - **K7 Top Spending Categories** (Filter view)
      - **K11 Largest Single Expense** (Highlight)
    - _Components:_ Searchable table, filters (Date, Category, Account), Bulk editing.
4.  **Insights (New)**
    - _Primary Goal:_ Behavioral analysis and trends.
    - _Included KPIs:_
