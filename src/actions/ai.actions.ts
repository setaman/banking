...
// ---------------------------------------------------------------------------
// Legacy (pre-v1.1) actions
//
// Kept as thin wrappers over the profile-based API above so the current
// settings card (`src/components/settings/ai-provider-card.tsx`) and
// `/assistant` page (`getAiConfigStatus` only) keep compiling unmodified.
// Both operate on the ACTIVE profile, matching the old single-config UX:
// there used to be exactly one configuration; now there's exactly one
// *active* profile from this component's point of view. Package 2 replaces
// the settings card with a true multi-profile UI built directly on the
// actions above, at which point these wrappers can be removed.
// ---------------------------------------------------------------------------
...
/** @deprecated Use `getAiProfilesStatus` (Package 2 settings UI). */
...
const LegacySaveAiConfigSchema = z.object({
  provider: AiProviderSchema,
  model: z.string().trim().min(1, "Model is required"),
  apiKey: z.string().trim().min(1).optional(),
  baseUrl: z.string().trim().url().optional(),
...
export type LegacyAiConfigInput = z.infer<typeof LegacySaveAiConfigSchema>;

/**
 * @deprecated Use `saveAiProfile` (Package 2 settings UI). Creates the
 * first profile, or fully overwrites the currently active profile's
 * provider/model/apiKey/baseUrl in place (same full-overwrite semantics as
 * the pre-v1.1 single-config `saveAiConfig` — not merge-safe, unlike the
 * new `saveAiProfile` action).
 */
export async function saveAiConfig(
  input: LegacyAiConfigInput
): Promise<ActionResult> {
  const parsed = LegacySaveAiConfigSchema.safeParse(input);
  if (!parsed.success) {
    return {
      success: false,
      error: parsed.error.issues.map((i) => i.message).join(", "),
  ...
  const profile: AiProfile = {
    ...
    name:
      active?.name ??
      deriveAiProfileName(parsed.data.provider, parsed.data.model),
    provider: parsed.data.provider,
